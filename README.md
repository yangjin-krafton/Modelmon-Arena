...
**픽셀 아트 스타일의 턴제 배틀 게임 프로토타입**입니다.

이 프로젝트는 원작 포켓몬 모바일 게임의 클래식한 픽셀 아트 감성을 기반으로,  
...
현대 AI 시스템들을 몬스터처럼 의인화/재해석한 픽셀 아트 배틀 게임입니다.

플레이어는 각기 다른 AI 특성을 가진 몬스터를 팀으로 편성하고,  
속성 상성, 상태이상, 스킬 조합, 시너지 효과를 활용해 전투를 진행합니다.

현재 개발은 **도감(Dex) 시스템**을 우선 구현하며,  
이를 통해 캐릭터 정체성, 데이터 구조, 아트 자산을 먼저 확정한 뒤  
전투 시스템으로 확장하는 순서를 따릅니다.
...
- 원작 포켓몬 모바일 게임의 클래식 픽셀 아트 스타일
- 완전 오리지널 AI 기반 세계관
- PixiJS 기반 2D 픽셀 아트 렌더링
- 도감 우선 개발 → 전투 시스템 확장 순서
...
- 원작 포켓몬 모바일 새로 화면 스타일의 클래식 픽셀 아트
- 16~32px 기반 스프라이트 캐릭터
- AI 모델 특징을 상징적으로 반영한 픽셀 아트 디자인
- 도감 화면: 레트로 UI + 픽셀 몬스터 카드
- 전투 화면: 2D 사이드뷰 픽셀 아트 배틀
- 복잡한 필드 탐험 없이 도감 열람과 배틀 연출 중심
...
- **PixiJS** — 2D 픽셀 아트 렌더링 엔진
- **HTML / CSS / JavaScript**
- **픽셀 아트 스프라이트** (PNG/WEBP)
- **CSV/JSON 기반 데이터 관리**
- **Local state battle system**
- 필요 시 **Aseprite** 등을 통한 스프라이트 제작
...
### Phase 1: 도감 (현재 진행)
- 모델몬 도감 시스템 구현
- 픽셀 아트 스프라이트 기반 도감 카드
- 시리즈별 진화 라인 표시
- AI 개념 필드 (핵심 개념, 입출력 방식, 파라미터 등)
- 도감 설명문 (한국어)
- 필터/검색 기능
